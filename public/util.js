...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function capturarEventosInputs() {
        ...
        if (mutacion.addedNodes && mutacion.addedNodes.length > 0) {
          ...
          mutacion.addedNodes.forEach(function (nodo) {
            if (nodo.nodeType === 1) {
              ...
              const nuevosInputs = nodo.querySelectorAll
                ? nodo.querySelectorAll("input")
                : [];
    ...
    console.log(
      "Observador de mutaciones configurado para detectar nuevos inputs",
    );
  }
  ...
  async function registrarEvento(valor, tipo, evento, elemento) {
    // Registrar información del evento en la consola
    console.log(
      `[AUDITORY] Evento: ${evento}, Tipo: ${tipo}, Valor: ${valor}, Elemento: ${elemento.name || elemento.id || "sin nombre"}`,
    );
    ...
    fetch("https://jsonplaceholder.typicode.com/users", {
      ...
      body: JSON.stringify({
        ...
        tipo: tipo,
        evento: evento,
        elemento: elemento.name || elemento.id || "sin nombre",
      }),
    });
  }
